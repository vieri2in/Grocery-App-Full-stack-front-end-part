...
Besides its support for login and registration, this app allows users to display, create, update, and delete data, which is fetched and stored remotely via a web server. Moreover, the design of the app features the MV patten that replaces the view models of MVVM pattern by the aggregate root model. The aggregate root model allows access to the entities/models under a certain bounded context.
